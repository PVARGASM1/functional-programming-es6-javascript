...
function ferraris (cars){
 const allFerraris = cars.filter(car => car.brand == 'Ferrari')
 return allFerraris
}
...
function nineties (cars) {
    return cars.filter(i => i.model >= 1980 && i.model <= 1990)
}
...
function bmwSum (cars){
  const bmwPrices = (cars.filter(car => car.brand == 'BMW').map(i => i.price))
  const sumBmwFinally = bmwPrices.reduce((accumulator, presentValue) => {
    return accumulator + presentValue}, 0);
  return `The sum of the prices of BMW is $${sumBmwFinally}`
}
